...
function MaigretTool() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/maigret/search', {
        method: 'POST',
        headers: {
      ...
      {result && (
        <div style={{ marginTop: '15px' }}>
          <div style={{
            ...
            ✓ Search completed successfully for: <strong>{result.username}</strong>
          </div>
        </div>
      )}
    </div>
